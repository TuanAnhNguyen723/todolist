document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".fa-eye").forEach((eyeIcon) => {
    eyeIcon.addEventListener("click", function () {
      ...
      const taskId = taskContainer
        .querySelector("input.form-checkbox")
        .getAttribute("data-task-id"); // Lấy task_id từ checkbox
...
// Bắt sự kiện khi người dùng nhập vào ô search
document.getElementById("search-input").addEventListener("input", function () {
  const query = this.value.toLowerCase(); // Chuyển đổi từ khóa thành chữ thường
  const tasks = document.querySelectorAll(".task-item"); // Lấy tất cả các task

  tasks.forEach((task) => {
    const title = task.getAttribute("data-title").toLowerCase(); // Lấy title của task

    // Kiểm tra xem title có chứa từ khóa tìm kiếm không
    if (title.includes(query)) {
      task.classList.remove("hidden"); // Hiển thị task nếu có trùng khớp
    } else {
