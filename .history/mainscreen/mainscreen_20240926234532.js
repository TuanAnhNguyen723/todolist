document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".fa-pencil").forEach((editIcon) => {
    editIcon.addEventListener("click", function (event) {
      ...
      // Lấy thông tin từ các thuộc tính của task (sử dụng closest để lấy form)
      const taskElement = this.closest(".task-container");
  
      if (taskElement) { // Kiểm tra nếu taskElement tồn tại
        const taskId = taskElement.querySelector("input[name='task_id']").value;
        const taskTitle = taskElement.querySelector(".task-text").innerText;
        const taskDescription = taskElement.querySelector("input[name='description']").value;
        const taskTimeStart = taskElement.querySelector("input[name='time_start']").value;
        const taskTimeEnd = taskElement.querySelector("input[name='time_end']").value;
  
        // Hiển thị thông tin trong modal edit
        document.querySelector("input[name='edit_task_id']").value = taskId;
        document.querySelector("input[name='edit_title']").value = taskTitle;
        ...
        console.error("Không tìm thấy phần tử cha chứa thông tin task.");
      }
    });
  });
  ...
  document
    ...
    .addEventListener("click", function (event) {
      ...
      // Lấy thông tin đã chỉnh sửa từ modal
      ...
      const timeEnd = document.querySelector(
        "input[name='edit_time_end']"
      ).value;

      // Gửi yêu cầu AJAX để cập nhật thông tin task
      fetch("mainscreenController.php", {
        method: "POST",
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
