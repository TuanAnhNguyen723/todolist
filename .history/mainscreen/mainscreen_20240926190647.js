...
document.addEventListener("DOMContentLoaded", () => {
  ...
  deleteButtons.forEach((button) => {
    button.addEventListener("click", async function (event) {
      ...
      const taskId = this.dataset.taskId; // Giả sử có thuộc tính data-task-id trên icon thùng rác

      // Hiển thị xác nhận trước khi xóa
      if (confirm("Bạn có chắc chắn muốn xóa nhiệm vụ này không?")) {
        // Gửi yêu cầu xóa qua AJAX
        const response = await fetch("./mainscreenController.", {
          method: "POST",
          headers: {
            "Content-Type": "application/x-www-form-urlencoded",
          },
        ...
        if (result.status === "success") {
          ...
          // Tải lại trang để cập nhật danh sách task
          window.location.reload();
        } else {
