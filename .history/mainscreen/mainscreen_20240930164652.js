...
document.addEventListener("DOMContentLoaded", () => {
  // Gán sự kiện click cho nút "Lưu" (保存)
  document.getElementById("saveEditButton").addEventListener("click", function (event) {
    ...
    // Gửi yêu cầu cập nhật task qua AJAX
    fetch("mainscreenController.php", {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: `task_id=${taskId}&edit_task=1&title=${encodeURIComponent(title)}&description=${encodeURIComponent(description)}&time_start=${timeStart}&time_end=${timeEnd}`,
    })
    .then((response) => response.text())
    .then((data) => {
      console.log(data); // Kiểm tra phản hồi từ server
      // Làm mới trang hoặc cập nhật giao diện
      window.location.reload(); // Sau khi cập nhật thành công, reload lại trang để thấy thay đổi
    })
    .catch((error) => console.error("Error:", error));
  });
