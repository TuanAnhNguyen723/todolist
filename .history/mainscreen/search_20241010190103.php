...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Kiểm tra xem dữ liệu tìm kiếm có được gửi không
    if (!empty($_POST['title'])) {
        // Sử dụng Prepared Statements để tránh SQL Injection
        $sql = "SELECT * FROM task WHERE title LIKE ? ORDER BY time_end DESC";
        $title = $_POST['title'];
        $stmt = $conn->prepare($sql);
        $likeTitle = $title . '%';
        $stmt->bind_param("s", $likeTitle);
    } else {
        // Trường hợp không có dữ liệu tìm kiếm, trả về tất cả dữ liệu và sắp xếp theo time_end DESC
