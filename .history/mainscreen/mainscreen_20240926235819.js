document.addEventListener("DOMContentLoaded", () => {
  ...
  function showTaskEditModal(task) {
    const { taskId, taskTitle, taskDescription, taskTimeStart, taskTimeEnd } =
      task;
  ...
  document.querySelectorAll(".fa-pencil").forEach((editIcon) => {
    editIcon.addEventListener("click", (event) => {
      ...
      // Lấy phần tử chứa thông tin task
      const taskElement = editIcon.closest(".task-container");

      if (taskElement) {
        // Lấy thông tin từ các thuộc tính của task
        ...
        const taskDescription =
          taskElement.querySelector(".task-description").innerText; // nếu bạn có phần tử chứa mô tả task
        const taskTimeStart = taskElement.querySelector(
          "input[name='time_start']"
        ).value;
        const taskTimeEnd = taskElement.querySelector(
          "input[name='time_end']"
        ).value;

        // Điền các thông tin vào modal edit
        document.querySelector("input[name='edit_task_id']").value = taskId;
        document.querySelector("input[name='edit_title']").value = taskTitle;
        document.querySelector("textarea[name='edit_description']").value =
          taskDescription;
        ...
        console.error("Không tìm thấy phần tử chứa thông tin task.");
      }
    });
  });
