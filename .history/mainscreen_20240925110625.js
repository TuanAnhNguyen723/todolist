...
document.querySelectorAll(".star-icon").forEach((star) => {
  star.addEventListener("click", function () {
    ...
    // Toggle màu ngôi sao
    this.classList.toggle("text-yellow-100");
    // Toggle màu chữ của task
    if (this.classList.contains("text-yellow-100")) {
      taskText.classList.add("text-yellow-100");
    } else {
      taskText.classList.remove("text-yellow-100");
    }
  });
});
